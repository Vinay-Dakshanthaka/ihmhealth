...
onAuthStateChanged(auth, async (user) => {
    ...
    if (user) {
        ...
        if (!localStorage.getItem('checkoutSummary')) {
            // Redirect to cart.html if cart information is not found
...
// Check if decryptedResponse query parameter exists in the URL
if (urlParams.has('decryptedResponse')) {
...
//         const userSnapshot = await getDoc(doc(firestore, 'users', auth.currentUser.uid));
//         const payerName = userSnapshot.data().firstName + " " + userSnapshot.data().lastName;
...
//         // Make a POST request to the backend to fetch the data
...
//         form.setAttribute('action', formData.spURL);
...
async function payment(e) {
    ...
    const addressCard = document.querySelector('.selected-address');
    if (!addressCard) {
        displayMessage('Please give an address!', 'danger');
        return;
    }
    const addressId = addressCard.getAttribute('data-id');
...
const addressData = addressSnapshot.data();
    ...
    try {
        ...
        // Construct the form data for SabPaisa
        const form = document.createElement('form');
        form.setAttribute('action', formData.spURL);
        form.setAttribute('method', 'POST');
        ...
        // Submit the form to initiate payment with SabPaisa
        form.submit();
